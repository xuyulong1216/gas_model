...
def dic_gen(k,v):
	if type(lis) != list :
		...
		return {k[i]:v[i] for i in range(0,len(k))}
...
class EnvDiscriptor:
	def __init__(self,name,cubes):
		...
		self.expr=0
		self.coords=[self.x,self.y,self.z]
		self.expr=esum([i.get_expr().replace({i.x:self.x,i.y:self.y,i.z:self.z}) for i in cubes])
		self.expr=self.expr.doit(deep=False)
		self.decrease=esum([self.get_reflect().replace({i.x:self.x,i.y:self.y,i.z:self.z}) for i in cubes])
...
class GaussModel:
	def __init__(self,name,source_position):
		...
		self.expr=self.expr.subs(dic_gen(space_coords,coord_convert(space_coords,[[1,0,0],[0,1,0],[0,0,1]],source_position,[[1,0,0],[0,1,0],[0,0,1]],[0,0,0]) ))
...
class GasMonitor:
	def __init__(self,data,position):  
		#data[data0,data1,data2....] ,30 lines at least ,newer data at top ,scalar value 
		#position:[x,y,z]
		self.data=data
